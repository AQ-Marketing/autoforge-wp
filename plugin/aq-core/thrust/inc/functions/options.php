...
function rocket_export_options() {
	...
	$filename  = sprintf( 'boost-settings-%s-%s-%s.json', $site_name, date( 'Y-m-d' ), uniqid() ); // phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date
...
function rocket_valid_key() {
	// Boost: the engine is unconditionally unlocked — no license, no SaaS.
	return true;

	$rocket_secret_key = (string) get_rocket_option( 'secret_key', '' ); // phpcs:ignore Squiz.PHP.NonExecutableCode -- unreachable by design.
...
function rocket_check_key() {
	// Boost: never call api.wp-rocket.me; the key is always considered valid.
	return true;

	// Recheck the license.
	$return = rocket_valid_key(); // phpcs:ignore Squiz.PHP.NonExecutableCode -- unreachable by design.
