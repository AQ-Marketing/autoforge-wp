...
function rocket_thank_you_license() {
	if ( '1' === get_rocket_option( 'license' ) ) {
		...
		// Boost: Rocket Insights is removed — always use the plain message.
...
function rocket_analytics_optin_notice() {
	// Boost: analytics/Mixpanel tracking is removed — never ask for opt-in.
	return;

	$screen = get_current_screen(); // phpcs:ignore Squiz.PHP.NonExecutableCode -- unreachable by design.
