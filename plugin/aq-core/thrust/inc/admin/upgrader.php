...
function rocket_upgrader() {
	...
	if (
		WP_ROCKET_PLUGIN_SLUG === $page
		&&
		did_action( 'wp_rocket_upgrade' )
	) {
		wp_safe_redirect( esc_url_raw( admin_url( 'options-general.php?page=' . WP_ROCKET_PLUGIN_SLUG ) ) );
		exit;
	}

	if ( ! rocket_valid_key() && current_user_can( 'rocket_manage_options' ) && WP_ROCKET_PLUGIN_SLUG === $page ) {
	...
	add_option(
		...
		apply_filters(
			...
			[
				...
				'cdn'                         => 0, // Boost: CDN module removed (Pressable edge handles delivery).
				'cdn_cnames'                  => [],
				'cdn_zone'                    => [],
				'cdn_reject_files'            => [],
				'do_cloudflare'               => 0,
