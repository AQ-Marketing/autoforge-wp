...
function rocket_add_admin_css_js() {
	...
	wp_localize_script(
		...
		apply_filters(
			...
			[
				...
				// Boost: was https://api.wp-rocket.me (postMessage origin for the
				// RocketCDN iframe, which is removed). Point it at the site itself.
				'origin_url' => home_url(),
			]
		)
	);
	...
	// Boost: brand override stylesheet — depends on wpr-admin and is enqueued last,
	// so its equal-specificity selectors win and survive any upstream CSS refresh.
	wp_enqueue_style( 'boost-admin', WP_ROCKET_ASSETS_CSS_URL . 'boost-admin.css', [ 'wpr-admin' ], WP_ROCKET_VERSION );
}
