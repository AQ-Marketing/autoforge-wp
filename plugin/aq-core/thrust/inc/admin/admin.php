...
function rocket_settings_action_links( $actions ) {
	...
	// Boost: no wp-rocket.me Support/Docs/FAQ links; just the settings shortcut.
	array_unshift( $actions, sprintf( '<a href="%s">%s</a>', admin_url( 'options-general.php?page=' . WP_ROCKET_PLUGIN_SLUG ), __( 'Boost Settings', 'rocket' ) ) );

	return $actions;
}
// Boost is embedded in AQ Core, so the link belongs on the aq-core plugin row.
add_filter( 'plugin_action_links_aq-core/aq-core.php', 'rocket_settings_action_links' );

// Boost: the "Renew your licence" plugin row-meta nag is removed (no license).
...
function rocket_handle_settings_import() {
	...
	if ( isset( $_FILES['import']['name'] ) && ! preg_match( '/(?:boost|wp-rocket)-settings(?:-.*)?-20\d{2}-\d{2}-\d{2}-[a-f0-9]{13}\.json/', sanitize_file_name( $_FILES['import']['name'] ) ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.MissingUnslash
