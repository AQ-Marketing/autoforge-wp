...
abstract class AbstractAPIClient {
	...
	public function get_exclusions_list( $hash ) {
		// Boost: remote exclusion-list API is disabled — bundled lists only.
		return [
			'code'    => 0,
			'message' => __( 'Remote list updates are disabled in Boost; bundled lists are used.', 'rocket' ),
		];

		$args = [ // phpcs:ignore Squiz.PHP.NonExecutableCode -- unreachable by design.
			'body'    => [
				'hash' => $hash,
			],
			'timeout' => 5,
