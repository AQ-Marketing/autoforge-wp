...
class DynamicLists extends Abstract_Render {
	...
	public function update_lists_from_remote() {
		// Boost: never call the WP Media exclusions API (b.rucss.wp-rocket.me).
		// "Updating" reloads the lists bundled with the plugin, so the weekly cron
		// and the Tools-tab button stay functional with zero external requests.
		$titles = [
		...
		/** This action is documented in inc/Engine/Optimization/DynamicLists/DynamicLists.php */
		do_action( 'rocket_after_save_dynamic_lists', true );
