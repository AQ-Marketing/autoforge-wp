...
function get_rocket_cdn_url( $url, $zone = [ 'all' ] ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals, Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	// Boost: the CDN module is removed (Pressable's edge handles delivery). This
	// public API wrapper is kept for backward compatibility and returns the URL
	// unchanged so any theme/plugin calling it never fatals.
	return $url;
}
