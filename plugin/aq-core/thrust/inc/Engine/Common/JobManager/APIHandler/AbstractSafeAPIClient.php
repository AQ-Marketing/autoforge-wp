...
abstract class AbstractSafeAPIClient {
	...
	private function send_request( $method, $params = [], $safe = false ) {
		// Boost: every subclass of this client targets a WP Media / wp-rocket.me
		// endpoint (user, pricing, remote settings, RocketCDN). All outbound calls
		// are disabled; callers already handle the WP_Error path gracefully.
		return new WP_Error( 503, 'Remote API calls are disabled in Boost.' );

		$api_url = $this->get_api_url(); // phpcs:ignore Squiz.PHP.NonExecutableCode -- unreachable by design.
		if ( empty( $api_url ) ) {
